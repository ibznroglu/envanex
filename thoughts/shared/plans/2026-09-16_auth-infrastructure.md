...
6. `RotatedAt is not null` → **reuse**: log the detection, then revoke the family with reason `Reuse` **through the same bounded retry as logout** (see `RevokeFamilyAsync` below), and return `RefreshTokenReused` whether this caller revoked the family itself or lost to the writer that did. Grace period zero (research decision 7). **The consumed row is never deleted** — that is what makes generation-1 replay detectable after three rotations. *(Changed from a plain revoke-and-save by the **Phase 4 db-review**: two replays of one token both reach this branch, both load the same live rows, and the second save matches zero — an unguarded revoke leaves a `DbUpdateConcurrencyException` on a path whose contract is `Result`, after "reuse detected" has already been logged.)*
7. `now >= ExpiresAt || now >= FamilyExpiresAt` → revoke the family's live rows with reason `Expired` **through the same bounded retry**, and return `RefreshTokenExpired`. *(Same **Phase 4 db-review** change, same race: one client with two tabs refreshing the same stored token past the idle window reaches this branch twice.)*
...
4. After a successful save, re-query for any remaining live row in the family. If one exists and attempts remain, loop. **After the loop, re-query once more before concluding anything**: a `DbUpdateConcurrencyException` on the *final* attempt skips the in-loop check, and the writer that won that race may already have revoked what was left — a family that is in fact dead. Only when a live row survives that final read is the revocation unfinished: log `Error` "Failed to revoke refresh token family {FamilyId} after {Attempts} attempts" and return `Result.Failure(AuthErrors.InvalidRefreshToken)`. **A logout that cannot guarantee revocation must not report success**, and equally, a failed write of this caller's own is never on its own proof that the family is alive. *(The loop and this `Error` line are reached from **all three** revocation sites — reuse, expiry and logout — not from logout alone; the post-loop re-read came from the **Phase 4 db-review**, advisory A1.)*
5. Otherwise `Result.Success()`.

The exhaustion branch is forced deterministically by `RefreshTokenRevocationRetryTests`, which makes every attempt lose through an `ISaveChangesInterceptor` registered in the test's own container — no seam in `src/`, and the only way to reach the branch, since a `DbUpdateConcurrencyException` on the *final* attempt cannot be ordered from two real racers. `RevokeFamilyAsync_WhenTheFinalAttemptLosesToAWriterThatRevokedTheFamily_ShouldReturnSuccess` covers the post-loop re-read: a winner revokes the family out of band on the final attempt, and the method returns `Success` with no `Error` line. `RevokeFamilyAsync_WhenEveryAttemptLosesAndTheFamilyIsStillLive_ShouldFailAndLogAnError` covers the genuine exhaustion, so the re-read cannot degenerate into always reporting success. Both assert `Attempts == RevocationRetryLimit`, so an interceptor registered too late to take effect fails the test rather than passing it. It is also covered at the handler level by `LogoutCommandHandlerTests.HandleAsync_RevocationFailure_ShouldPropagateTheFailure`, and its intended outcome is asserted by `RefreshTokenConcurrencyTests.RevokeFamilyAsync_RacingARotation_ShouldLeaveNoLiveRowInTheFamily`. *(Was "cannot be forced deterministically and has no dedicated integration test"; the tests that falsify it came from the **Phase 4 db-review**, advisory A1.)*
