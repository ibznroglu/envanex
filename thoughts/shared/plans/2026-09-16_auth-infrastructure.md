...
- The family lock: filtered unique index `IX_RefreshTokens_FamilyId_Live` **plus** the `RowVersion` token — and **which of the two loser exits actually fires**, per the Phase 4 tally. Revision 4's analysis says the unique-violation exit is unreachable through `RotateAsync` by construction and is kept as defence in depth; the ADR records the measured answer.
...
- **(G) EF Core statement ordering inside `SaveChangesAsync` — the sharpest risk in this phase.** Rotation stamps the parent (`RotatedAt`) and inserts the child. If the INSERT reached the server **before** the UPDATE, the parent would still be live at insert time and `IX_RefreshTokens_FamilyId_Live` would reject **every** rotation, not just a racing one. Revision 4 removes the dependency on EF's internal command ordering: see step 8 below, which specifies an explicit transaction with two saves in a fixed order. **Settled by:** the first green run of `RotateAsync_ValidToken_ShouldReturnADifferentTokenString` plus the logged SQL pasted into the report, which must show `UPDATE ... RefreshTokens ... SET RotatedAt` before `INSERT ... RefreshTokens`.
...
8. Otherwise, **inside one explicit transaction** (`await using var tx = await context.Database.BeginTransactionAsync(ct)`), in this order:
   a. `parent.MarkRotated(now, childId)` where `childId` is the pre-generated `Guid` of the child; `SaveChangesAsync` — this is the write whose `RowVersion` predicate decides the race, and it takes the row lock;
   b. add the child built by `parent.CreateChild(...)`; `SaveChangesAsync`;
   c. `tx.CommitAsync(ct)`.
   **Revision 4 replaced revision 3's single `SaveChangesAsync`** because a single save leaves the UPDATE/INSERT order to EF's internal batch ordering, and an INSERT-first batch would make the parent still live at insert time and trip `IX_RefreshTokens_FamilyId_Live` on *every* rotation (risk G). Two saves in one transaction are still atomic — a failure leaves neither the stamp nor the child — and the order is now stated rather than inherited.
9. **Loser exits, both returning `InvalidRefreshToken`:**
   - `DbUpdateConcurrencyException` from step 8a — another rotation or a revocation already changed the parent's `RowVersion`. **This is the reachable one:** both racers read the same parent and both try to stamp it, and the stamp precedes the insert.
   - `DbUpdateException` whose inner is `SqlException { Number: 2601 or 2627 }` from step 8b. Detect it with the pattern `UnitOfWork` already uses. **This exit is defence in depth and no test in this plan reaches it** — see "the unique-violation exit" below. Do **not** route either through `IUnitOfWork`, which is bound to `EnvanexDbContext`.
...
**The unique-violation exit, stated so nobody writes a test that pretends to cover it.** For the child INSERT to violate `IX_RefreshTokens_FamilyId_Live`, a second live row must exist in the family at insert time. The parent is stamped first, under a `RowVersion` predicate, so a racing rotation loses at step 8a and never reaches 8b; and no legal database state has two live rows in one family, because that is exactly what the index forbids — which means the state cannot be constructed by a test either. The exit is kept because it converts an unexpected infrastructure surprise (or a future code path that inserts a child without stamping a parent, such as a pruning job) into the same clean 401 rather than a 500. The index itself is protected by the deterministic Phase 2 test `RefreshTokens_InsertingASecondLiveTokenForTheSameFamily_ShouldThrowUniqueViolation`. The Phase 4 report tallies which exit actually fires; the ADR records the answer.
...
- `RotateAsync_TwoConcurrentRotationsOfTheSameToken_ShouldEndInOneOfTheTwoAcceptedStates` — **replaces revision 3's three strict loser assertions** (finding 2). Per iteration, exactly one result succeeds and the non-winner's error code decides which post-state must hold:
  - `Auth.InvalidRefreshToken` (the concurrency exit) → exactly **one** live row; the parent carries `RotatedAt` and a `ReplacedByTokenId` equal to that live row's `Id`; no row in the family carries `RevokedAt`.
  ...
  **What its failure means:** removing either catch in step 9 turns it red on the interleaving that reaches that catch (an escaped `DbUpdateConcurrencyException` is not one of the accepted states); dropping `MarkRotated`'s `ReplacedByTokenId` stamp turns it red on the concurrency branch. It does **not** guard the reuse branch itself — `RotateAsync_ReplayOfGenerationOneAfterASingleRotation_ShouldReturnRefreshTokenReused` does that deterministically — and it does **not** guard `IX_RefreshTokens_FamilyId_Live`, which the Phase 2 schema test guards.
...
3. **A tally of the concurrent-rotation loser exits** across all iterations: how many ended in `DbUpdateConcurrencyException`, how many in a 2601/2627 unique violation, how many in reuse detection. If the unique violation never fires — which the analysis above predicts — say so plainly; that exit is then untested defence in depth and the ADR records it as such. If it *does* fire, say so: the analysis is wrong and the human needs to know.
...
5. The `dotnet list --include-transitive` output for risk (A).
...
- `docs/roadmap.md` — modified — mark PR 6a done; record the gaps, first one as **BLOCKER**:
  ...
  - The 2601/2627 loser exit in `RotateAsync` is defence in depth with no test that reaches it (Phase 4 analysis).
...
3. **Two things in Phase 4 deserve your ruling before the ADR.** (a) The 2601/2627 loser exit in `RotateAsync` is unreachable by construction and is kept as defence in depth with no test that reaches it; you may prefer to delete it instead. (b) Rotation now uses an explicit transaction with two saves in a stated order rather than one `SaveChangesAsync`, so the correctness of every rotation does not depend on EF Core's internal command ordering. Both are recorded above and both belong in the ADR.
