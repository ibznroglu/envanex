...
```bash
dotnet tool restore            # the pinned EF Core CLI
cp .env.example .env           # then set MSSQL_SA_PASSWORD in .env; the file explains the rules
docker compose up -d --wait    # SQL Server 2022, bound to 127.0.0.1:1433 only; waits for the healthcheck
...
With the demo off, the app starts but has no account to sign in with, so the demo steps below are how you sign in locally.
